...
- **Seating chart**: the persisted assignments rendered as a large-format (A2, A1 selectable) list — each table with its seated guests (seat order), in site style with inward-pointing floral corner decorations, sized for reading at arm's length. Browser prints to PDF; print shop takes the PDF.
