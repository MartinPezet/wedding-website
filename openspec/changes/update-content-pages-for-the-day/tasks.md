...
- [x] 1.2 Make `ScheduleEvent.end` and `mapsUrl` optional and drop their empty values from `schedule.json`; omit the end property in `server/utils/ics.ts` when an event has no end time; drop `url` and `linkText` from `gifts.json` and the `Gifts` type; update the gifts page SEO description
